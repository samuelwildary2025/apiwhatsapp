...
function InstanceDetailContent() {
    ...
    const loadSettings = async () => {
        try {
            ...
            if (response.data) {
                ...
                const data = response.data as InstanceSettings;
                setProxyForm({
                    host: data.proxyHost || '',
                    port: data.proxyPort || '',
    ...
    const saveProxy = async () => {
        setSavingProxy(true);
        try {
            await api.updateInstanceSettings(id, {
                proxyHost: proxyForm.host,
                proxyPort: proxyForm.port,
                proxyUsername: proxyForm.username,
                proxyPassword: proxyForm.password,
                proxyProtocol: proxyForm.protocol
            });
